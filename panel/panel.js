...
(function () {
  ...
  function renderPanel() {
    ...
    $('#tg-flows').value = JSON.stringify(c.flows || [], null, 2);
    ...
    $('#tg-runbook').value = c.lastRunbookMd || 'Aún no se ha generado el RUNBOOK.';
  ...
  function saveContext() {
    var flows;
    try {
      flows = JSON.parse($('#tg-flows').value || '[]');
      if (!Array.isArray(flows)) throw new Error('debe ser una lista');
    } catch (e) {
      return flash('err', 'Los flujos no son JSON válido: ' + e.message);
    }
